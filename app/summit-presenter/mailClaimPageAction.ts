...
import { z } from 'zod';
import { getPopulatedEmail as getPopulatedEmail } from './mail-template';
...
export async function sendEmail(prevState: State, formData: FormData) : Promise<State> {
  ...
  try {
    ...
    const emailContent = getPopulatedEmail(collectionPageURL, validatedFields.data.recipientName)
